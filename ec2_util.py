...
def get_instance_type(instance):
    ...
    paravirtual (PV) or hardware virtual machine (HVM). The main difference between
    PV and HVM AMIs is the way in which they boot and whether they can take advantage
    of special hardware extensions (CPU, network, and storage) for better performance.

    :param instance: str name of instance
    :return: instance virtualization type
    """
...
def get_existing_cluster(conn, cluster_name, die_on_error=True):
    ...
    if any((master_nodes, slave_nodes)):
        print(u'Found {0:d} master(s), {1:d} slaves'.format(len(master_nodes), len(slave_nodes)))
